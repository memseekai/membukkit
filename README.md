...
The same no-LLM treatment on conversational memory: session-level Recall@5 on the original LongMemEval-S split, the metric [gbrain](https://github.com/garrytan/gbrain-evals) and MemPalace publish against. With gbrain's own embedding model (`text-embedding-3-large`) under MemBukkit's turn-level indexing and no reranker at all, **99.8%** (499/500) against gbrain-hybrid's 97.6% and MemPalace's 96.6%, same dataset, same K, same n, no LLM in the loop, none of our fine-tuned weights involved. The as-shipped production cut with the same encoder scores 99.2%; with our 110M local encoder and zero API calls, 96.4%. Per-question rankings, a training-overlap disclosure for the shipped reranker, and the commands that regenerate every row are in [`benchmarks/longmemeval_session_recall/`](https://github.com/memseekai/membukkit/blob/main/benchmarks/longmemeval_session_recall/README.md).
